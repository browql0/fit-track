...
import { Apple, ChevronRight, Coffee, Droplets, Globe2, Info, Moon, PencilLine, Plus, Search, Sparkles, Sun, Trash2, UtensilsCrossed, Clock } from 'lucide-react';
...
export const Nutrition = () => {
  ...
  return (
    ...
    >
      ...
      <Modal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); resetAssistant(); }} title={selectedFood ? 'Details' : 'Ajouter un aliment'}>
        {!selectedFood ? (
          <div className="nutri-modal-search">
            ...
            {searchQuery && searchResults.length === 0 && (
              <AssistPanel
                ...
                startManual={() => { setAssistMode('manual'); setDraftFood({ ...emptyDraft, name: searchQuery }); }}
              />
            )}

            {assistantError && <div className="error-panel">{assistantError}</div>}
            {assistMode === 'external' && <ExternalResults loading={externalLoading} results={externalResults} setSelectedFood={setSelectedFood} />}
            {assistMode === 'estimate' && (
          ...
          <form className="nutri-modal-form" onSubmit={handleAddEntry}>
            <div className="nutri-food-card">
              <div className="nutri-food-card-icon"><Info size={24} /></div>
